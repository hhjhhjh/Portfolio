...
이 문서에서 PoC는 개념을 검증하는 초기 구현, STT는 음성을 텍스트로 변환하는 기술을 뜻합니다. API는 서비스 사이의 호출 규약이며, SDK는 여러 서비스가 그 규약을 같은 방식으로 사용하도록 묶은 코드입니다.
...
- Redis 기반 잠금과 대기 흐름, 파일 캐시 중복 탐지, 재시도와 장애 시 fallback을 구현했습니다.
...
- 외부 AI 서비스는 시간 제한과 일시적 실패를 전제로 설계해야 하며, 캐시와 fallback은 부가 기능이 아니라 핵심 운영 요구사항입니다.
- LLM 기능을 운영하려면 프롬프트뿐 아니라 작업 상태, 파일 수명주기, 데이터 계보와 장애 복구를 함께 설계해야 합니다.
- 여러 도메인이 같은 인프라를 사용할 때는 공통 SDK의 계약과 오류 모델이 개별 기능의 개발 속도와 안정성을 좌우합니다.
- 금융 도메인의 생성형 AI는 사용자가 결과의 근거를 검토하고 확정하는 human-in-the-loop 흐름이 필요합니다.
