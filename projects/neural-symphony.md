...
- 담당 범위: Beethoven 데이터의 LSTM·Transformer 모델 실험과 생성 결과 제작
...
Beethoven 곡을 대상으로 이벤트 기반 MIDI 시퀀스를 학습하는 LSTM과 Decoder-only Transformer 노트북을 구축하고 실험했습니다. 두 모델이 동일한 작곡가 데이터에서 생성한 결과를 비교할 수 있도록 학습과 생성 흐름을 정리했습니다.
...
아래는 팀 전체가 사용한 접근이며, 제 직접 담당 범위는 Beethoven 데이터의 모델 실험과 생성 결과 제작입니다.
...
- 생성 모델에서는 학습 모델만큼 입력 표현과 디코딩 전략이 결과를 결정한다는 점을 확인했습니다.
