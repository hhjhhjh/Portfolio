...
- 직접 기여: Random Forest와 LightGBM 모델 실험을 담당했습니다.
- 결과: 팀의 대표 out-of-sample 실험에서는 XGBoost가 약 0.116의 Sharpe Ratio를 기록했습니다.
...
- 프로젝트: 감사보고서의 정량 표는 SQL로, 정성 본문은 Vector DB로 검색하는 금융 문서 질의응답 시스템입니다.
- 직접 기여: 팀 프로젝트에서 HTML 보고서의 본문·표 분리, SQL 조회 실험과 프로젝트 문서화를 담당했습니다.
...
- 직접 기여: Beethoven 데이터의 모델 실험을 담당했습니다.
...
- 결과: LightGBM의 임계값을 0.50에서 0.825로 조정해 precision을 0.3095에서 0.4882로, F1을 0.4583에서 0.5598로 개선하고 recall 감소와의 trade-off를 확인했습니다.
...
- 프로젝트: 얼굴 이미지에서 피부 트러블을 탐지하고 얼굴 부위별 정보를 제공하는 웹 서비스입니다.
- 직접 기여: 모델 후처리와 Flask 서버, 데이터 저장, 결과 화면과 기록 기능을 담당했습니다.
- 결과: 이미지 업로드부터 분석 결과 조회까지 이어지는 프로토타입을 구현했습니다.
...
- 직접 기여: 데이터 통합과 EDA, 선형·트리 모델 비교, 최종 결과 분석을 담당했습니다.
...
- 직접 기여: 동별 안전도 군집화, 매물 주변 교통·안전 변수 결합과 안전점수 산출을 담당했습니다.
